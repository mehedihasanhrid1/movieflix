...
export default function Cart() {
  ...
  const cartItems = JSON.parse(localStorage.getItem('bookings')) || [];
    ...
    <div className="p-4 px-3 md:px-10 lg:px-20 mx-auto my-8 lg:mt-16">
      <div className="bg-white shadow-lg rounded-lg">
        ...
        {cartItems.map((item, index) => (
          <div key={index} className="py-5 pl-4 md:py-6 lg:py-8  md:pl-6 lg:pl-8 md:pr-4">
            <div className="flex items-center mb-4">
              <img className="h-20 w-20 lg:h-32 lg:w-32 object-contain rounded-lg mr-4 lg:mr-6" src={item.bookingimage} alt={item.movie} />
              <div className="flex-1">
                <h2 className="text-lg lg:text-2xl font-bold mb-1 lg:mb-2">{item.movie}</h2>
                <div className="flex gap-3 lg:gap-4 items-center flex-wrap text-gray-600 text-sm md:text-lg lg:text-xl lg:mb-2 font-semibold">
              ...
              <button
                className="text-red-500 text-xl lg:text-3xl font-bold p-2 mr-1 rounded-lg"
        ...
        <div className="px-4 md:px-6 py-6 md:py-8 bg-gray-200 rounded-b-lg">
          ...
          <button className="block w-full mt-4 bg-red-500 hover:bg-red-600 md:text-lg lg:text-xl text-white font-bold py-2.5 lg:py-3.5 rounded">
            Checkout
          </button>
        </div>
      </div>
...
Swal.fire({
  ...
  localStorage.setItem('bookings', JSON.stringify(updatedCart));
